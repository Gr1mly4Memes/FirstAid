...
public class DamageablePart extends AbstractDamageablePart {
    ...
    public CompoundTag serializeNBT() {
        ...
        if (activeHealer != null) {
            // Persist the healer stack as plain id + count. ItemStack has no NBT helpers without a
            // HolderLookup.Provider, and healer stacks never carry components, so this is sufficient.
            Identifier itemId = BuiltInRegistries.ITEM.getKey(activeHealer.stack.getItem());
            if (itemId != null) {
                CompoundTag healerTag = new CompoundTag();
                healerTag.putString("id", itemId.toString());
                healerTag.putInt("count", activeHealer.stack.getCount());
                compound.put("healer", healerTag);
                compound.putInt("itemTicks", activeHealer.getTicksPassed());
                compound.putInt("itemHeals", activeHealer.getHealsDone());
            } else {
                FirstAid.LOGGER.warn("Failed to lookup item id for healer stack {}", activeHealer.stack.getItem());
            }
        }
        return compound;
    }
    ...
    public void deserializeNBT(@Nullable CompoundTag nbt) {
        ...
        if (nbt.contains("maxHealth") && FirstAidConfig.SERVER.scaleMaxHealth.get())
            maxHealth = nbt.getInt("maxHealth").orElse(maxHealth);
        currentHealth = Math.min(maxHealth, nbt.getFloat("health").orElse(0F));
        ItemStack stack = null;
        if (nbt.contains("healer")) {
            Tag healerRaw = nbt.get("healer");
            if (healerRaw instanceof CompoundTag healerTag) {
                String idString = healerTag.getString("id").orElse(null);
                Identifier itemId = idString == null ? null : Identifier.tryParse(idString);
                Item item = itemId == null ? null : BuiltInRegistries.ITEM.getValue(itemId);
                if (item != null)
                    stack = new ItemStack(item, healerTag.getInt("count").orElse(1));
            }
        }
        ...
        if (stack != null) {
            ...
            if (healer == null) {
                ...
                activeHealer = healer.loadNBT(nbt.getInt("itemTicks").orElse(0), nbt.getInt("itemHeals").orElse(0));
            }
        }
        if (nbt.contains("absorption"))
            absorption = nbt.getFloat("absorption").orElse(0F);
