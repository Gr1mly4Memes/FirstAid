...
public class PlayerSizeHelper {
    ...
    public static IDamageDistributionAlgorithm getMeleeDistribution(Player player, DamageSource source) {
        ...
        if (causingEntity != null && causingEntity == source.getDirectEntity() && causingEntity instanceof Mob) {
            ...
            if (mobEntity.getTarget() == player && mobEntity.goalSelector.getAvailableGoals().stream().filter(WrappedGoal::isRunning).anyMatch(prioritizedGoal -> prioritizedGoal.getGoal() instanceof MeleeAttackGoal)) {
