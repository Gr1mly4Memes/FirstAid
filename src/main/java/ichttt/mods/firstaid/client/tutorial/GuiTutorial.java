...
public class GuiTutorial extends Screen {
    ...
    @Override
    public void init() {
        parent.init(this.width, this.height);
        guiTop = parent.guiTop - 30;
        addRenderableWidget(Button.builder(Component.literal(">"), button -> {
            if (action.hasNext()) GuiTutorial.this.action.next();
            else {
                minecraft.getConnection().send(new ServerboundCustomPayloadPacket(new MessageClientRequest(MessageClientRequest.Type.TUTORIAL_COMPLETE)));
    ...
    public void drawOffsetString(GuiGraphicsExtractor guiGraphics, String s, int yOffset) {
        guiGraphics.text(minecraft.font, s, parent.guiLeft + 30, guiTop + yOffset, 0xFFFFFFFF);
    }

    @Override
    public void extractRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTicks) {
        parent.extractRenderState(guiGraphics, mouseX, mouseY, partialTicks);
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, HealthRenderUtils.SHOW_WOUNDS_LOCATION, parent.guiLeft, guiTop, 0.0F, 139.0F, GuiHealthScreen.xSize, 28, 256, 256);
        this.action.draw(guiGraphics);
        guiGraphics.centeredText(minecraft.font, I18n.get("firstaid.tutorial.notice"), parent.guiLeft + (GuiHealthScreen.xSize / 2), parent.guiTop + 128, 0xFFFFFFFF);
